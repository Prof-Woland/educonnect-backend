...
export class AuthService {
    ...
    constructor(private readonly prismaService: PrismaService, private readonly configService: ConfigService, private readonly jwtService: JwtService, @Inject(CACHE_MANAGER) private cacheManager: Cache){
    ...
    async registration(dto: UserDto, res: Response){
        ...
        const returnUser = await this.prismaService.auth.findFirst({
            where:{
                email
            },
            select:{
    ...
    private generateTokens(id: string, role: string){
        ...
        const access_token = this.jwtService.sign(payload, {expiresIn: this.JWT_ACCESS_TOKEN_TTL, secret: this.JWT_SECRET});
        const refresh_token = this.jwtService.sign(payload, {expiresIn: this.JWT_REFRESH_TOKEN_TTL, secret: this.JWT_SECRET});

        this.logger.log(`Successful: ${id}`, this.name);
        return {access_token, refresh_token}
    }

    private async createCache(id: string, accessToken: string,  refreshToken: string){
